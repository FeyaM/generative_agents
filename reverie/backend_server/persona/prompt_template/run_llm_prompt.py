...
from persona.prompt_template.model_structure import *
...
##############################################################################
# CHAPTER 1: Run LLAMA3_8B Prompt
##############################################################################

model_name = "llama3_8B"

def run_llama3_8b_prompt_wake_up_hour(persona, test_input=None): 
  """
  给定角色信息，返回角色起床的时间，返回一个整数
  ...
  def __func_clean_up(model_response, prompt=""):
    cr = int(ast.literal_eval(model_response)["wake_up_hour"])
    logging.info(f'wake_up_hour: {cr}')
    return cr
  
  def __func_validate(model_response, prompt=""): 
    try: __func_clean_up(model_response, prompt="")
    except: return False
    return True
  ...
  prompt_template = "persona/prompt_template/v4_llama3/wake_up_hour_v1.txt"
  prompt_input = create_prompt_input(persona, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
  logging.info(f'prompt: {prompt}')
  fail_safe = get_fail_safe()

  output = safe_generate_response(model_name, prompt, None, 5, fail_safe,
                                   __func_validate, __func_clean_up)
  
  logging.info(f'output: {output}')
  
  return output, [output, prompt, fail_safe]


def run_llama3_8b_prompt_daily_plan(persona, 
                              wake_up_hour, 
                              test_input=None):
  """
  基本上是跨越一天的长期计划。返回角色今天要采取的行动列表。通常采用以下形式： 
  早上 6:00 起床并完成晨练“、”早上 7:00 吃早餐"、.....、 
  早上 7:00 吃早餐 "等。
  请注意，这些行动都不带句号。
  ...
  def __func_clean_up(model_response, prompt=""):
    cr = ast.literal_eval(model_response)["daily_req"]
    # 检查 cr 是不是一个列表
    if not isinstance(cr, list): 
      logging.error(f'daily_req is not a list: {cr}')
      return False
    return cr
  ...
  prompt_template = "persona/prompt_template/v4_llama3/daily_planning_v1.txt"
...
def run_llama3_8b_prompt_generate_hourly_schedule(persona, 
                                            curr_hour_str,
                                            p_f_ds_hourly_org, 
                                            hour_str,
                                            intermission2=None,
  ...
  输入:
    persona: Persona 实体类
    curr_hour_str: 当前小时字符串
    p_f_ds_hourly_org: 原始计划
    hour_str: 小时字符串
    intermission2: 插曲
    test_input: 测试输入
  ...
  def create_prompt_input(persona, 
    ...
    if p_f_ds_hourly_org: 
      ...
      for count, i in enumerate(p_f_ds_hourly_org): 
        ...
        prior_schedule += f" {hour_str[count]}] {{\"activity\": \"{persona.scratch.get_str_firstname()} is {i}\"}}\n"

    prompt_ending = f"[(ID:{get_random_alphanumeric()})"
    prompt_ending += f" {persona.scratch.get_str_curr_date_str()} -- {curr_hour_str}]"
  ...
  def __func_clean_up(model_response, prompt=""):
    cr = int(ast.literal_eval(model_response)["activity"])
    logging.info(f'activity: {cr}')
    return cr
  ...
  prompt_template = "persona/prompt_template/v4_llama3/generate_hourly_schedule_v1.txt"
...
def run_llama3_8b_prompt_task_decomp(persona, 
                               task, 
                               duration, 
                               test_input=None, 
                               verbose=False): 
  ...
  def __func_clean_up(model_response, prompt=""):
    # 提取任务描述列表
    task_descriptions = model_response["task_decomp"]
        # 检查 task_descriptions 是不是一个列表
    if not isinstance(task_descriptions, list): 
      logging.error(f'task_decomp is not a list: {task_descriptions}')
      return False
    # 初始化任务列表
    cr = []
    ...
    for task_desc in task_descriptions:
        # 分割任务描述以提取任务名称和持续时间
        task_parts = task_desc.split("(duration in minutes: ")
        task_name = task_parts[0].strip()
        if task_name[-1] == ".":  # 如果任务名称以句点结尾，去除句点
            task_name = task_name[:-1]
        duration = int(task_parts[1].split(",")[0].strip())

        # 添加任务名称和持续时间到任务列表
        cr.append([task_name, duration])
  ...
  def __func_validate(model_response, prompt=""): 
    ...
    try: 
      __func_clean_up(model_response)
    except: 
      pass
      # return False
  ...
  prompt_template = "persona/prompt_template/v4_llama3/task_decomp_v1.txt"
  prompt_input = create_prompt_input(persona, task, duration)
  prompt = generate_prompt(prompt_input, prompt_template)
  fail_safe = get_fail_safe()

  output = safe_generate_response(model_name, prompt, None, 5, get_fail_safe(),
                                   __func_validate, __func_clean_up)
...
def run_llama3_8b_prompt_action_sector(action_description, 
                                persona, 
                                maze, 
                                test_input=None, 
                                verbose=False):
  ...
  def __func_clean_up(model_response, prompt=""):
    cr = ast.literal_eval(model_response)["area"]
    logging.info(f'area: {cr}')
    return cr
  ...
  prompt_template = "persona/prompt_template/v4_llama3/action_location_sector_v1.txt"
...
def run_llama3_8b_prompt_action_arena(action_description, 
                                persona, 
  ...
  prompt_template = "persona/prompt_template/v4_llama3/action_location_object_v1.txt"
...
def run_llama3_8b_prompt_action_game_object(action_description, 
                                      persona, 
                                      maze,
                                      temp_address,
                                      test_input=None, 
  ...
  def __func_clean_up(model_response, prompt=""):
    cr = ast.literal_eval(model_response)["object"]
    logging.info(f'object: {cr}')
    return cr
  ...
  prompt_template = "persona/prompt_template/v4_llama3/action_object_v1.txt"
...
def run_llama3_8b_prompt_pronunciatio(action_description, persona, verbose=False):
  """
  用于获取动作的emoji表示
  ...
  def __func_clean_up(model_response, prompt=""):
    cr = ast.literal_eval(model_response)["emojis"]
    logging.info(f'emojis: {cr}')
    return cr
  ...
  prompt_template = "persona/prompt_template/v4_llama3/generate_pronunciatio_v1.txt"
  prompt_input = create_prompt_input(action_description)
  prompt = generate_prompt(prompt_input, prompt_template)

  fail_safe = get_fail_safe()
  output = safe_generate_response(model_name, prompt, None, 3, fail_safe,
                                          __func_validate, __func_clean_up)

  logging.info(f"output: {output}")


def run_llama3_8b_prompt_event_triple(action_description, persona, verbose=False):
  """
  用于获取事件的三元组
  ...
  def __func_clean_up(model_response, prompt=""):
    # 将字典转换为列表
    cr = [model_response["predicate"], model_response["object"]]
    return cr

  def __func_validate(model_response, prompt=""): 
    try: 
      model_response = __func_clean_up(model_response, prompt="")
  ...
  prompt_template = "persona/prompt_template/v4_llama3/generate_event_triple_v1.txt"
...
def run_llama3_8b_prompt_act_obj_desc(act_game_object, act_desp, persona):
  """
  用于获取交互对象的描述

  输入:
    act_game_object: 动作对象
    act_desp: 动作描述
    persona: 角色实例
  ...
  prompt_template = "persona/prompt_template/v4_llama3/generate_obj_event_v1.txt"
...
def run_llama3_8b_prompt_act_obj_event_triple(act_game_object, act_obj_desc, persona): 
  """
  用于获取活动对象事件的三元组
...
def run_llama3_8b_prompt_new_decomp_schedule(persona, 
                                       main_act_dur, 
                                       truncated_act_dur, 
                                       start_time_hour,
                                       end_time_hour, 
  ...
  输入:
    persona: 角色实例
    main_act_dur: 主活动列表
    truncated_act_dur: 截断后的活动列表
    start_time_hour: 开始时间
    end_time_hour: 结束时间
    inserted_act: 插入的活动
    inserted_act_dur: 插入的活动时长
